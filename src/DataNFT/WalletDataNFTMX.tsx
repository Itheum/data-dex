...
import { Box, Text, Link, Image, Flex, Skeleton, Popover, PopoverTrigger, PopoverContent, PopoverHeader, PopoverArrow, PopoverCloseButton, PopoverBody, Badge, Button, HStack, NumberInput, NumberInputField, NumberInputStepper, NumberIncrementStepper, NumberDecrementStepper, useToast, useDisclosure, Modal, ModalBody, ModalContent, ModalHeader, ModalOverlay, Stack, Alert, AlertDescription, AlertIcon, AlertTitle, CloseButton, ModalCloseButton, Spinner } from "@chakra-ui/react";
...
export default function WalletDataNFTMX(item: WalletDataNFTMxPropType) {
  ...
  return <Box key={item.id} maxW="xs" borderWidth="1px" borderRadius="lg" overflow="wrap" mb="1rem" position="relative" w="13.5rem">
    ...
    <Flex h="28rem" p="3" direction="column" justify="space-between">
      ...
      <Box>
        ...
        <Button
          ...
          List {amount} NFT{amount > 1 && "s"} for{" "}
          {price ? `${price} ITHEUM ${amount > 1 ? "each" : ""}` : "Free"}
        </Button>
      </Box>
    </Flex>
    ...
    <Box
      ...
      rounded="lg"
      visibility={
        item.userData && (item.userData?.addressFrozen || (item.userData?.frozenNonces && item.userData?.frozenNonces.includes(item?.nonce))) ? "visible" : "collapse"
      }
      backdropFilter="auto"
      backdropBlur="6px">
