  ...
  const fetchAccountSignature = async (message: string) => {
    ...
    if (walletUsedSession === 'el_webwallet') { // web wallet not supported
      ...
      try {
        ...
        // if (signatureObj?.signature?.buffer && signatureObj?.address) { // Maiar App V2 / Ledger
        ...
        // }
        if (signatureObj?.signature && signatureObj?.address) { // Maiar App V2 / Ledger
          signResult.addrInHex = signatureObj.address.hex();
            ...
            dataNfts.map((item, index) => (
              <Box key={item.id} maxW="xs" borderWidth="1px" borderRadius="lg" overflow="hidden" mr="1rem" mb="1rem" position="relative" w="15.5rem">
                ...
                <Box
                  ...
                  visibility={userData && (userData.addressFrozen || (userData.frozenNonces && userData.frozenNonces.includes(item.nonce))) ? "visible" : "collapse"}
                >
                  <Text
                    fontSize="md"
                    position="absolute"
